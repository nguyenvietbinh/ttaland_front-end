...
const Property_type = ({ isForRent, isForSale, setIsForRent, setIsForSale} : Property_type_props) => {

  return (
    <div className="flex flex-col gap-2 w-full">
      <p className="text-2xl font-bold">Nhu Cầu</p>
      <div className="flex gap-4 w-full">
        <div onClick={() => {setIsForSale(true); setIsForRent(false)}} className={`p-4 cursor-pointer ${isForSale ? 'border-2 border-black shadow-md' : 'border-1 border-gray-300'} hover:bg-gray-100 rounded-3xl w-full`}>
        ...
        <div onClick={() => {setIsForRent(true); setIsForSale(false)}} className={`p-4 cursor-pointer ${isForRent ? 'border-2 border-black shadow-md' : 'border-1 border-gray-300'} hover:bg-gray-100 rounded-3xl w-full`}>
