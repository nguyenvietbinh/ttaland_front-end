'use client'


const New_card = () => {

  return (
    <div className="flex w-full mt-4 cursor-pointer">
      <img className="w-1/3 rounded-lg" src="/img/background.jpg" alt="" />
      <div className="w-2/3 pl-2">
      <p className="text-sm text-gray-500">25/09/2025</p>
        <p className="text-2xl line-clamp-2 hover:underline">Lộ Diện Đại Lý Phân Phối F1 Chiến Lược Dự Án Vinhomes Green Paradise Cần Giờ</p>
        <p className="text-base text-gray-400 line-clamp-3 hover:underline">SSM Group chính thức trở thành Đối tác phân phối F1 Chính thức dự án Vinhomes Green Paradise Cần Giờ – siêu đô thị biển mang tầm vóc quốc tế, mở ra cơ hội đầu tư mới đầy tiềm năng tại TP.HCM.</p>
      </div>
    </div>
  )
}

export default New_card